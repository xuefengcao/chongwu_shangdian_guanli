...
public class UserController extends BaseController {
	...
	@GetMapping("/getupdatePwd")
	String getupdatePwd( Model model) {
		UserDO userDO =getUser();
		UserDO userDO1 = userService.get(userDO.getUserId());

		model.addAttribute("user", userDO1);
		return prefix + "/update_pwd";
	}
	@PostMapping("/updatePwd")
	@ResponseBody
	R updatePwd(UserDO user) {
		UserDO userDO = userService.get(user.getUserId());
		String oldpassword = MD5Utils.encrypt(userDO.getUsername(), user.getOldpassword());
		if(!userDO.getPassword().equals(oldpassword)){
			return R.error("原始密码不正确！");
		}
		String password = MD5Utils.encrypt(userDO.getUsername(), user.getPassword());
